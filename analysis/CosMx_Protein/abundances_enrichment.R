...
long.data <- meta[meta$tissue %in% c("IBD","NHC"),]


freq.table <- long.data %>%
  dplyr::group_by(tissue, patient, subset) %>%
  dplyr::tally()

chis.table <- table(long.data$tissue, long.data$subset)
...
for (i in 1:ncol(chis.table)) {
  ...
  wilx <- wilcox.test(n ~ tissue, data = freq.table[
    freq.table$subset == colnames(chis.table)[i],
  ])
  ...
  df <- data.frame(row.names = colnames(chis.table)[i],
...
df_final$ast_wlx <- ifelse(df_final$FC > 1.5 |df_final$FC < 0.66,
                           yes =ifelse(df_final$fdr_wlx < 0.05, no = '',
                                       yes = ifelse(df_final$fdr_wlx < 0.01, no = '*',
                                                    yes = ifelse(df_final$fdr_wlx < 0.001,
                                                                 no = '**',
                                                                 yes = '***'))),
                           no = '')

df_final$fdr_Chisq <- p.adjust(df_final$Chisq_p.value, method = 'BY')
df_final$ast_Chisq <- ifelse(df_final$FC > 1.5 |df_final$FC < 0.66,
                             yes =ifelse(df_final$fdr_Chisq < 0.05, no = '',
                                         yes = ifelse(df_final$fdr_Chisq < 0.01, no = '*',
...
long.data <- meta[meta$tissue %in% c("PD","NHC"),]
...
df_final1$ast_wlx <- ifelse(df_final1$FC > 1.5 |df_final1$FC < 0.66,
                            yes =ifelse(df_final1$fdr_wlx < 0.05, no = '',
                                        yes = ifelse(df_final1$fdr_wlx < 0.01, no = '*',
                                                     yes = ifelse(df_final1$fdr_wlx < 0.001,
                                                                  no = '**',
                                                                  yes = '***'))),
                            no = '')

df_final1$fdr_Chisq <- p.adjust(df_final1$Chisq_p.value, method = 'BY')
df_final1$ast_Chisq <- ifelse(df_final1$FC > 1.5 |df_final1$FC < 0.66,
                              yes =ifelse(df_final1$fdr_Chisq < 0.05, no = '',
                                          yes = ifelse(df_final1$fdr_Chisq < 0.01, no = '*',
...
write.csv(df_final,"analysis/CosMx_Protein/Results/enrichment.csv")
