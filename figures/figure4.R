...
all_int$refined_receptor <-
  mapvalues(all_int$id_receptor,
            from = meta$cell_id,
            to = meta$refined)
all_int$refined_source   <-
...
chordiagram <- function(chord,
                        meta_seu,
                        fov = "all",
                        subset_source = "all",
                        subset_target = "all",
                        refined_source = "all",
                        refined_receptor = "all",
                        freq = 1,
                        down = FALSE) {
  if ("all" %in% subset_source)
    subset_source <- unique(meta_seu$subset)
  if ("all" %in% subset_target)
    subset_target <- unique(meta_seu$subset)
  ...
  chord <-
    as.data.frame(table(chord$refined_source, chord$refined_receptor))
...
plot_spatial_connections <-
  function(seurat_obj,
    ...
    network <- BiocParallel::bplapply(cells, BPPARAM = bp, \(c) {
      ...
      tt <-
        sqrt((meta$CenterX_global_px - x) ^ 2 + (meta$CenterY_global_px - y) ^
               2)
      cn <- rownames(meta)
      your_dataframe <-
        data.frame(Column1 = cn, Column2 = as.vector(tt))
    ...
    # Connection matrix
    lines_df <-
      BiocParallel::bplapply(1:length(network), BPPARAM = bp, \(c) {
        conn <- network[[c]]
        start_cell <- conn[1]
    ...
    big_df$cell_start <-
      ifelse(big_df$cell_start == "Colonocytes", "Colonocytes", "Other")
    big_df <- big_df %>%
      mutate(
        cell_start = case_when(
          cell_start == "Colonocytes" & fert_end == "pos" ~ "Colonocytes+",
          cell_start == "Colonocytes" &
            fert_end == "neg" ~ "Colonocytes-",
          TRUE ~ cell_start
        )
      )
    ...
    p <- ggplot() +
      ...
      geom_segment(
        data = big_df[big_df$cell_start == "Colonocytes+",],
        aes(
          x = x_start,
          y = y_start,
          xend = x_end,
      ...
      geom_segment(
        data = big_df[big_df$cell_start == "Other",],
        aes(
          x = x_start,
          y = y_start,
          xend = x_end,
      ...
      labs(
        title = paste0("Spatial Connection Map - FOV ", fov_id),
        x = "X Coordinate",
        y = "Y Coordinate"
      ) +
...
df_filtered <- all_int %>%
  filter(refined_source %in% colonocyte_types |
           refined_receptor %in% colonocyte_types)
...
interaction_counts <- df_filtered %>%
  ...
  filter(cell_type == "Colonocytes_FERT+" &
           tissue %in% c("IBD", "NHC", "PD"))
...
ggsave(
  "figures/plots/fig4B_boxplot.png",
  plot = pB,
  width = 6,
  height = 4,
  dpi = 300
...
plot_spatial_connections(seurat_obj = seurat,
                         fov_id = 26,
                         output_file = "figures/plots/fig4C_fov26.png")
plot_spatial_connections(seurat_obj = seurat,
                         fov_id = 10,
                         output_file = "figures/plots/fig4Ci_fov10.png")
plot_spatial_connections(seurat_obj = seurat,
                         fov_id = 13,
                         output_file = "figures/plots/fig4Cii_fov13.png")
#D - Volcano plots -------------------------------------------------------------
pC <-
  volcano(
    anot = "refined",
...
ggsave(
  "figures/plots/fig4C_PDvsNHC.png",
  plot = pC,
  width = 6,
  height = 4,
  dpi = 300
...
ggsave(
  "figures/plots/fig4D_IBDvsNHC.png",
  plot = pD,
  width = 6,
  height = 4,
  dpi = 300
