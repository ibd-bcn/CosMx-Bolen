...
seurats_rna  <- readRDS("analysis/CosMx_RNA/Objects/seurats_all_norm.RDS")
...
enrichment   <- read.csv("analysis/CosMx_Protein/Results/enrichment.csv")
all_int      <- read.csv("analysis/CosMx_Protein/Results/all_int.csv")
# ============================================================================ #
#   Helper functions
# ============================================================================ #
...
plot_pol <- function(object,
  ...
  if (fov != "all") object <- object[object$fov == fov, ]
  cells <- object$cell
...
refined_col <- c(
  # T cells
  "MT T cells" = "#5050FFFF", "CD8" = "#CE3D32FF", "Cycling T cells" = "#802268FF",
  "ILC4" = "#749B58FF", "NK" = "#466983FF", "T cells CCL20" = "#BA6338FF",
  "Tregs" = "#F0E685FF", "CD4" = "#5DB1DDFF", "MAIT" = "#6BD76BFF",
  "Ribhi T cells" = "#D595A7FF", "gd IEL" = "#00FFFFFF", "DN" = "#7A65A5FF",
  # Plasmas
  "PC IgG" = "#CC9900FF", "Memory B cell" = "#99CC00FF", "Cycling cells" = "#FF1463FF",
  "PC IgA" = "#0000CCFF", "PC IgA heat shock"= "#3B1B53FF", "PC IER" = "#CCCC99FF",
  "Naïve B cell" = "#FF0000FF", "B cell" = "#F7B6D2FF", "GC B cell" = "#990080FF",
  # Epithelium
  "Secretory progenitor" = "#FFFF00FF", "Epithelium Ribhi" = "#FF7F0EFF",
  "Cycling TA" = "#C75127FF", "Colonocytes" = "#9EDAE5FF",
  "Inflammatory colonocyte" = "#9467BDFF", "BEST4 OTOP2" = "#33CC00FF",
  "Goblet" = "#CC0000FF", "Enteroendocrine" = "#003399FF",
  "Tuft cells" = "#FFC20AFF", "Paneth-like" = "#FF00CCFF",
  # Myeloids
  "M2" = "#ECFF00", "M1" = "#FFCCCCFF", "M0" = "#924822FF",
  "DCs" = "#489C97", "Mast" = "#FF5733FF", "Cycling myeloid" = "#2E8B57FF",
  "IDA macrophage" = "#8A2BE2FF", "Inflammatory monocytes" = "#FF1493FF",
  "Neutrophil" = "#00FA9AFF", "Eosinophils" = "#FFD700FF",
  # Stroma
  "Endothelium" = "#4682B4FF", "Myofibroblasts" = "#7FFF00FF", "Pericytes" = "#00CED1FF",
  "S3" = "#BDB76BFF", "Fibroblasts" = "#556B2FFF", "Glia" = "#32CD32FF",
  "Inflammatory fibroblasts" = "#924822FF", "S1" = "#FF5733FF", "FRCs" = "#8A2BE2FF"
)
...
# Akoya
pA1 <- plot_fov(object = akoya, fov = "all", annotation = "Classification")
ggsave("figures/plots/fig6A_Akoya.png", plot = pA1, width = 12, height = 9, dpi = 300)

# RNA polygons
pA2 <- plot_pol(object = seurats_rna@meta.data, fov = 36, poly = pols_rna,
                annotation = "subset", pal = refined_col)
ggsave("figures/plots/fig6A_RNA.png", plot = pA2, width = 12, height = 9, dpi = 300)

# Protein polygons
pA3 <- plot_pol(object = seurats_prot@meta.data, fov = 36, poly = pols_prot,
                annotation = "subset", pal = proti)
ggsave("figures/plots/fig6A_Prot.png", plot = pA3, width = 12, height = 9, dpi = 300)
...
ggsave("figures/plots/fig6B_UMAP.png", plot = pBi, width = 12, height = 9, dpi = 300)
...
ggsave("figures/plots/fig6C_Enrichment.png", plot = pC, width = 12, height = 9, dpi = 300)
...
all_int$tissue <- mapvalues(all_int$id_source, seurats_prot@meta.data$cell_id, seurats_prot@meta.data$tissue)
...
for (cond in conditions) {
  ...
  chord_plot_prot(meta_seu = meta, mat = mat, cell_type = "Epithelium", color_chord = color_chord)
  dev.off()
}
##E - Violin plots -------------------------------------------------------------
...
for(g in gene){
  ...
  # Open PNG device
  png(filename = out_file, width = 4, height = 6, units = "in", res = 600)
  ...
  p <- ggplot(meta_tt, aes(x = tissue, y = ccl18, fill = tissue)) +
    ...
    labs(x = 'Health', y = paste0('log2 ', g, ' fluorescence'), title = g) +
    ...
    stat_compare_means(comparisons = list(c("IBD", "NHC"),
                                          c("NHC", "PD"),
                                          c("IBD", "PD")),
                       method = "wilcox.test",
